...
**管理画面には production window が出ないこと。** Debug は管理 PC のキーボードで各 content window にフォーカスして `D` → `P`。
