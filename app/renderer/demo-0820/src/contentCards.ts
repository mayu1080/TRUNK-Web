...
function fallbackResult(error: string): ContentCardLoadResult {
  const cards = createPlaceholderCards(demoConfig.targetCardCount);
  return {
    cards,
    realImageCount: 0,
    sourceImageCount: 0,
...
export async function loadContentCards(): Promise<ContentCardLoadResult> {
  ...
  try {
    ...
    const target = demoConfig.targetCardCount;
    ...
    console.info('[0820] content cards', {
      exploreSource,
    ...
    console.error('[0820] content card load failed', err);
