...
import { DEMO_PHASE, DEMO_VARIANT, demoConfig } from './demoConfig';
    ...
    (async () => {
      try {
        ...
        await window.trunkApi.logEvent({
          level: 'info',
          message: '0820-color demo renderer ready',
          context: { phase: DEMO_PHASE, variant: DEMO_VARIANT, screenState: state.screenState },
        });
      } catch (err) {
        if (!cancelled) {
          setBootError(err instanceof Error ? err.message : String(err));
