...
/** Screen-space bubble reveal uniforms（全 image mesh で共有） */
...
/**
 * MeshBasicMaterial の map_fragment 後に:
 * 通常は白黒、screen-space 円内だけカラー復帰。
 */
export function attachBubbleRevealShader(
  ...
  material.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      /* 縁はハード（添付イメージ相当）。アンチエイリアス用に極薄い羽だけ */
      float edge = 1.0;
